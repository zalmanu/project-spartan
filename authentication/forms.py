...
class LoginForm(forms.Form):
    ...
    password = forms.CharField(max_length=100, widget=forms.PasswordInput(attrs={'required':'required'}), label='Password')

class RegisterForm(forms.Form):
    username = forms.CharField(max_length=30, label='Username',widget=forms.TextInput(attrs={'required':'required'}))
    email = forms.CharField(max_length=100, label='Email', widget=forms.EmailInput(attrs={'required':'required'}))
    password = forms.CharField(max_length=160, min_length=8, label='Password', widget=forms.PasswordInput(attrs={'required':'required'}))
    password2 = forms.CharField(max_length=160,min_length=8, label='Retype password', widget=forms.PasswordInput(attrs={'required':'required'}))
...
class PasswordResetForm(forms.Form):
    oldpass = forms.CharField(max_length=160, min_length=8, label="Old password", widget=forms.PasswordInput(attrs={'required':'required'}))
